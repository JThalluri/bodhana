...
const PAGE_W_MM = 8.5 * 25.4;
const PAGE_H_MM = 11 * 25.4;
const CELL_MM = 8;
...
export function buildWorksheetsUI(container) {
  container.innerHTML = `
    <div class="ws-tool tool-shell">
      ...
      <div class="ws-body tool-body">
        <div class="ws-settings-pane tool-settings no-print">
          ...
          <div class="ws-settings-section">
            <div class="ws-section-title">Practice Text</div>
          ...
          <div class="ws-settings-section">
            <div class="ws-section-title">Geometry</div>

            <div class="ws-field" title="Font size in px (28-50). Default 38 px is calibrated for Andika on the 8 mm Seyes grid.">
              <label for="wsFontSize">Font px</label>
              <input class="tb-num" type="number" id="wsFontSize" min="28" max="50" step="0.5" value="38">
            </div>

            <div class="ws-field" title="Baseline offset in mm (-6 to +12). Nudge text up or down to seat characters on the grid line.">
              <label for="wsBaseline">Baseline mm</label>
              <input class="tb-num" type="number" id="wsBaseline" min="-6" max="12" step="0.1" value="0">
            </div>

            <div class="ws-field" title="Left margin must be a multiple of 8 mm so characters align to grid columns.">
              <label for="wsLeftPad">Left margin</label>
            ...
            <div class="ws-field" title="Blank grid rows between text lines (0-8). Default 3 gives learners 3 rows to copy each line.">
              <label for="wsRowGap">Blank rows</label>
              <input class="tb-num" type="number" id="wsRowGap" min="0" max="8" step="1" value="3">
            </div>

            <div class="ws-field" title="Top margin in mm (0-24). Offsets text from the paper top edge to avoid printer clipping.">
              <label for="wsTopPad">Top margin mm</label>
              <input class="tb-num" type="number" id="wsTopPad" min="0" max="24" step="0.5" value="8">
            </div>
          </div>

          <div class="ws-settings-section">
            <div class="ws-section-title">Print</div>
        ...
        <div class="ws-pages-pane tool-preview">
          <div class="tool-preview-scroll">
            <div id="wsPagesContainer" class="tool-pages"></div>
          </div>
        </div>
  ...
  function update() {
    ...
    if (pageCountEl) {
      pageCountEl.textContent = pages.length === 1 ? '1 page' : `${pages.length} pages`;
    }
